<template>
    <div :class="[ubicacion,'parametro']" :style="{backgroundColor:tipo === 'respuesta'?'#17C964':''}">
        <div class="container">
            <div>
                {{nombre}}
            </div>
            <div>
                <div v-show="tipo === 'respuesta'" class="icon-container">
                    <vs-icon v-show="tipo === 'respuesta'" class="iop-icon" icon="delete"></vs-icon>
                </div>
                <div v-show="tipo === 'entrada'" class="char-container">
                    <span class="ubicacion">{{ubicacion.charAt(0)||'b'}}</span>
                </div>
            </div>
        </div>
    </div>
</template>
<script>
export default {
    props:{
        tipo:String,
        nombre:String,
        ubicacion:String
    }
}
</script>
<style scoped>
.parametro{
    background-color: red;
    margin: 4px 4px;
    display: inline-block;
    width: auto;
    padding: 4px 4px 4px 10px;
    border-radius: 15px
}

.container{
    color: white;
    font-size: 11.6px;
    display: flex;
}
.container >div:first-child{
    display: flex;
    align-items: center;
    
}
.ubicacion{
    color: white;
    font-weight: 700;
    font-size: 9px;
    text-transform: uppercase;
}
.icon-container{
    width: 20px;
    height: 20px;
    border-radius: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
    background-color: rgba(0, 0, 0, .2);
    margin-left: 3px;
    cursor: pointer;
}
.char-container{
    width: 20px;
    height: 20px;
    border-radius: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
    background-color: rgba(100, 50, 0, .3);
    margin-left: 3px;
}
.icon-container:hover{
    background-color: var(--red);
}
.iop-icon{
    font-size: 14px;
}
.header{
    background-color: var(--orange);
}
.body{
    background-color: var(--red);
}
.query{
    background-color: var(--indigo);
}
.patha{
    background-color: var(--purple);
}/* 
.respuesta{
    background-color: var(--green);
} */
</style>
